Public Class Form3
    ...
    Private Sub Hitung_Click(sender As Object, e As EventArgs) Handles Hitung.Click
        Try
            Dim barMat1 As Integer = mat1.Lines.Count
            Dim kolMat1 As Integer = mat1.Lines.ElementAt(0).Split(" ").Length
            Dim barMat2 As Integer = mat2.Lines.Count()
            Dim kolMat2 As Integer = mat2.Lines.ElementAt(0).Split(" ").Length
        ...
        If (lihatkolom) Then
            For baris As Integer = 0 To brs1 - 1
                For kolom As Integer = 0 To kol1 - 1
                    nilai1(baris, kolom) = mat1.Lines.ElementAt(baris).Split(" ").ElementAt(kolom)
                    nilai2(baris, kolom) = mat2.Lines.ElementAt(baris).Split(" ").ElementAt(kolom)
                Next
            Next
        End If
    End Sub
    ...
    Private Sub cek(ByVal brs1 As Integer, brs2 As Integer, kol1 As Integer, kol2 As Integer)
        ...
        For baris = 0 To brs1 - 2
            If (mat1.Lines.ElementAt(baris).Split(" ").Length <> mat1.Lines.ElementAt(baris + 1).Split(" ").Length) Then
                tmp = False
            End If
        Next
        For baris = 0 To brs2 - 2
            If (mat2.Lines.ElementAt(baris).Split(" ").Length <> mat2.Lines.ElementAt(baris + 1).Split(" ").Length) Then
                tmp = False
            End If
        Next
        If (tmp = False) Then
    ...
    Private Sub operasi(ByVal matriks1(,) As Integer, matriks2(,) As Integer, batBaris As Integer, batKolom As Integer)
        Dim hasil As String = ""
        If (lihatkolom) Then
            Select Case ComboBox1_operasi.Text
                Case "Tambah"
